...
impl MobileFaceNet {
    pub fn new(model_path: &str) -> Result<Self, FacePamError> {
        let actual_path = if std::path::Path::new(model_path).exists() {
            ...
            "src/models/mobilefacenet/mobilefacenet.onnx" // Fallback local
        };
        ...
        let session = Session::builder()
            ...
            .map_err(|e| {
                FacePamError::Model(format!(
                    "Failed to load MobileFaceNet from {}: {}",
                    actual_path, e
                ))
            })?;
...
impl FaceRecognizer for MobileFaceNet {
    fn get_embedding(
        &mut self,
        image: &[u8],
        width: u32,
        ...
        let inputs = ort::inputs!["input.1" => val];
        let outputs = self
            .session
            .run(inputs)
            .map_err(|e| FacePamError::Model(format!("ONNX execution failed: {}", e)))?;

        let (_, data): (_, &[f32]) = outputs["516"].try_extract_tensor::<f32>().map_err(|e| {
